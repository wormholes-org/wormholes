...
func TestClient_SendTransaction(t *testing.T) {
	...
	log.Info("pre fromKey=", fromKey)
	log.Info("pub fromKey=", fromAddr.Hex())

	key := "3cc476e9a0d37a9de309a70dfad0daab2db7ffe1d2ae88e03d72fdd255b469a1"
	pub, pri, _ := PriKeyToAddress(key)
	proxy := pub.Hex()

	log.Info("proxy private", pri)
	log.Info("proxy key", proxy)
...
func testCallContract(t *testing.T, client *rpc.Client) {
	...
	if err != nil {
		log.Info("unexpected error: %v", err)
	}
	if gas != 21000 {
		log.Info("unexpected gas price: %v", gas)
	}
	// CallContract
...
func testAtFunctions(t *testing.T, client *rpc.Client) {
	...
	if pending != 1 {
		log.Info("unexpected pending, wanted 1 got: %v", pending)
	}
	// Query balance
	...
	if penNonce != nonce+1 {
		log.Info("unexpected nonce: %v %v", nonce, penNonce)
	}
	// StorageAt
	...
	if !bytes.Equal(storage, penStorage) {
		log.Info("unexpected storage: %v %v", storage, penStorage)
	}
	// CodeAt
