		...
		case <-w.emptyTimer.C:
			{
				...
				*/
				...
				if w.totalCondition < 120 && curBlock.Number().Uint64() > 0 {
					...
					log.Info("ok empty condition 15",
						"totalCondition", w.totalCondition,
						"height", new(big.Int).Add(w.chain.CurrentHeader().Number, big.NewInt(1)),
						"time", curTime, "blocktime", int64(w.chain.CurrentBlock().Time()),
						"online len", len(w.engine.OnlineValidators(curBlock.Number().Uint64()+1)))
				} else {
